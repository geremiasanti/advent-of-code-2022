...
impl<T> Stack<T> {
    ...
    fn pop_block(&mut self, n: isize) -> Vec<T> {
        let mut popped_vec = vec![];
        for _ in 0..n {
            let popped_element = self.stack.pop().unwrap_or_else(
                || panic!("Popped from empty stack")
            );  
            popped_vec.push(popped_element);
        }
        popped_vec.reverse();
        return popped_vec;
    }

    fn push_block(&mut self, mut to_push: Vec<T>) {
...
fn main() {
    ...
    for movement in movements {
        ...
        let popped = match stacks.entry(from) {
            ...
            Entry::Occupied(mut stack) =>
                stack.get_mut().pop_block(amount.try_into().unwrap()),
        };
        ...
        match stacks.entry(to) {
            ...
            Entry::Occupied(mut stack) =>
                stack.get_mut().push_block(popped),
